...
"""
╔══════════════════════════════════════════════════════════════════╗
║           STOCK MARKET PREDICTOR v2.0  (Enhanced)               ║
║                                                                  ║
║  NEW in v2:                                                      ║
...
def print_banner():
    if HAS_RICH:
        console.print(Panel.fit(
            "[bold cyan]📈 STOCK MARKET PREDICTOR v2.0 📉[/]\n"
            "[dim]Technical · Macro · ML · LSTM · Sentiment · Backtesting[/]",
            border_style="bright_blue",
        ))
    else:
        print("\n" + "=" * 62)
        print("  📈  STOCK MARKET PREDICTOR v2.0")
        print("  Technical · Macro · ML · LSTM · Sentiment · Backtesting")
        print("=" * 62)
...
class DataFetcher:
    ...
    # Macro tickers to download alongside the target stock
    MACRO_TICKERS = {
        "^TNX":    "US10Y_Yield",      # 10-Year Treasury Yield
        "^FVX":    "US5Y_Yield",       # 5-Year Treasury Yield
        "^IRX":    "US3M_Yield",       # 3-Month T-Bill
    ...
    def fetch_macro(period: str = "2y") -> pd.DataFrame:
        """Download macro indicator data and return as a single DataFrame."""
        rprint("  🌍  Fetching macro indicators (bonds, VIX, FX, commodities) ...")
        tickers = list(DataFetcher.MACRO_TICKERS.keys())
        try:
            ...
            close.columns = [DataFetcher.MACRO_TICKERS.get(c, c) for c in close.columns]
...
class MacroFeatures:
    ...
    def compute(macro_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "VIX" in macro_df:
            ...
            # VIX regime: low(<15), normal(15-25), high(25-35), extreme(>35)
...
class MLPredictor:
    ...
    def train_and_predict(self, df: pd.DataFrame, macro_feat: pd.DataFrame = None) -> dict:
        if not HAS_SKLEARN:
            raise ImportError("scikit-learn required: pip install scikit-learn")

        X, y, feature_names = self.prepare_data(df, macro_feat)
        if len(X) < 100:
...
class Backtester:
    ...
    def run(self, df: pd.DataFrame, macro_feat: pd.DataFrame = None,
        ...
        if macro_feat is not None and not macro_feat.empty:
            ...
            all_features = [c for c in tech_features if c in combined.columns] + available_macro
        else:
            combined = df
            all_features = [c for c in tech_features if c in combined.columns]
...
def print_report(symbol, market, tech, macro, ml, lstm, sentiment, ensemble, risk, backtest, df):
...
def main():
    ...
    if args.skip_macro:
        ...
        try:
            macro_df = DataFetcher.fetch_macro(args.period)
    ...
    if args.skip_ml or not HAS_SKLEARN:
        ...
        try:
            ml_result = MLPredictor().train_and_predict(df, macro_feat)
        except Exception as e:
    ...
    if args.skip_sentiment or not HAS_TEXTBLOB:
        ...
        rprint("\n  💬  Analyzing sentiment ...")
        sent_result = SentimentAnalyzer.analyze(DataFetcher.get_ticker(args.symbol, args.market))

    # 7. Ensemble
    ensemble = EnsembleCombiner.combine(
        technical=tech_signal, macro=macro_signal,
        ml=ml_result, lstm=lstm_result, sentiment=sent_result,
    )
    ...
    print_report(
        ...
        tech_signal, macro_signal, ml_result, lstm_result, sent_result,
