...
def print_banner():
    if HAS_RICH:
        console.print(Panel.fit(
            "[bold cyan]🌐 글로벌 시장 옴니(Omni) 예측기 v4.0 📈[/]\n"
            "[dim]거시 체제 · 7대 엔진 진단 · ML 앙상블 · 시장 리스크 관리[/]",
            border_style="bright_blue",
        ))
    else:
        print("\n" + "=" * 62)
        print("  🌐  글로벌 시장 옴니(Omni) 예측기 v4.0")
        print("  거시 체제 · 7대 엔진 진단 · ML 앙상블 · 시장 리스크 관리")
        print("=" * 62)
...
class MLPredictor:
    ...
    def train_and_predict(self, df: pd.DataFrame,
        ...
        if len(X) < 100:
            raise ValueError("머신러닝 학습을 위한 데이터가 부족합니다 (최소 100행 필요).")

        rprint(f"  🤖  머신러닝(ML) 앙상블 학습 중 ({len(feature_names)}개 피처, {len(X)}개 샘플) ...")
...
def main():
    parser = argparse.ArgumentParser(
        description="글로벌 시장 옴니(Omni) 예측기 v4.0 — 거시 체제 진단 및 다중 모델 결합 시장 분석 도구",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=dedent("""\
        분석 예시:
          python v4/market_predictor_v4.py ^GSPC          (S&P 500 지수 분석)
          python v4/market_predictor_v4.py ^IXIC          (나스닥 지수 분석)
          python v4/market_predictor_v4.py ^KS11          (코스피 지수 분석)
          python v4/market_predictor_v4.py QQQ --period 5y (나스닥 100 ETF 장기 분석)
          python v4/market_predictor_v4.py SPY --backtest-days 500 (S&P 500 ETF 백테스트)
        """),
    )
